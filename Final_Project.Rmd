...
ggplot(data = airbnb_cleaned, aes(x = price)) +
  ...
  geom_vline(xintercept = round(median(airbnb$price), 2), size = 1, color = 'red') 
```
...
df1 <- airbnb_cleaned$total_price
df2<- airbnb_cleaned$price

df_1 <- df1 %>%  mutate(Type = 'price with cleaning fees') %>%
       bind_rows(df2 %>%
           mutate(Type = 'price'))
...
ggplot() + geom_density(data = airbnb_cleaned, aes(x = total_price))+
  geom_density(data = airbnb_cleaned, aes(x = price), color = 'blue') +
    scale_x_log10() + legend() +
