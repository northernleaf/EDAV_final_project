...
ggplot(temp,aes(x = price, y = cleaning_fee, color = neighbourhood_group_cleansed)) + 
...
```{r}
temp <- dplyr::summarise(group_by(airbnb_cleaned, property_type, neighbourhood_group_cleansed), freq = n())  %>% filter(freq > 100)
...
correlation between review_scores_value and review_scores_accuracy, review_scores_cleanliness,review_scores_checkin,review_scores_communication,review_scores_location
