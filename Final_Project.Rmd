...
is 100. The boxplot in the following also shows that most of the listings have a price
under 200 dollars. 


```{r echo=FALSE, message = FALSE, warning = FALSE}
#boxplot of price
ggplot() + geom_boxplot(data = airbnb_cleaned %>% filter(price< 500), aes(x = "price", y = price))+
  ggtitle("Boxplot of Price (< 500)")
```  
  
### What Makes Some Listings Most Expensive?


There are some listings have ridiculous high prices. What are these listings? Why
are they so expensive? Let's further explore these lisitngs with high prices. 
