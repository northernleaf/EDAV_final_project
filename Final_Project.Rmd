...
Finally, there are 28098 listings and 62 variables for our analysis. The variables 
includes 
...
plot1 <- high_price %>%
  ...
  ggplot() + geom_bar(aes(x = reorder(neighbourhood_group_cleansed, counts), y = counts), 
                      stat = 'identity') + xlab("neighbourhood") + ggtitle('Neighbourhood counts of listing with 600+ price')
...
plot2 <- high_price %>%
 ...
 ggplot() + geom_bar(aes(x = reorder(neighbourhood_cleansed, counts), y = counts), 
                      stat = 'identity') + xlab("neighbourhood") + 
...
plot2 <- high_price %>%
  ...
  arrange(desc(counts)) %>% 
    ggplot() + geom_bar(aes(x = reorder(bedrooms, counts), y = counts), 
                      stat = 'identity') + xlab('number of bedrooms')+
...
market. These findings does not violate our common sense. 
...
ggplot(df2,
aes(x = neighbourhood_group_cleansed, y = prop*100, fill = property_type)) +
...
## Host 
### Does host verification affects review_scores_rating
### Does host_has_profile_pic affects review_scores_rating
### Dose host response rate affects review_scores_rating
### Dose host response time affects review_scores_rating
