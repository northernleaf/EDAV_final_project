...
plot2 <- high_price %>%
  ...
  arrange(desc(counts)) %>% 
    ggplot() + geom_bar(aes(x = reorder(bedrooms, counts), y = counts), 
                      stat = 'identity') + xlabel('number of bedrooms')
...
```{r}
#pairwise scatterplot between price and room type, neighbourhood, number_of_reviews,
#review_scores_rating, minimum_night, review_scores_location, 
