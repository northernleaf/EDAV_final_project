...
library(dplyr)
library(ggplot2)
data <- read_csv("/Users/amber/Downloads/Columbia Related/5702 HW/EDAV_final_project/data/data_cleaned.csv")
...
```{r}
## map for listings
data_map_all <- data %>% select(id, longitude, latitude)
...
data_map_room <- data %>% select(id, longitude, latitude, room_type)
...
ny_map_price <- get_map(location = sbbox1, maptype = "satellite", source = "google")
ggmap(ny_map_price) + 
